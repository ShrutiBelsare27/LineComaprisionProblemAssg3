...
{
   public  class CalculateToCompareToAndEquals
    {
        public void check()
        {
            
            CalculateToCompareToAndEquals obj = new CalculateToCompareToAndEquals();
            Console.WriteLine("if get 0:(both are equal),if get 1:(length 1 is grater than length2)," +
            "if get -1:(length 1 is lesser than length2) :: " +obj.computeMethode());
